...
createApp({
    data() {
        return {
            ...
            stats: {
                ...
                avgLoadTime: 0, euShieldedCount: 0
            },
            pagination: { page: 1, pages: 1, total: 0, limit: 100 },
            filters: { search: '', device: 'ALL', platform: 'ALL', browser: 'ALL', flag: 'ALL', dateFrom: '', dateTo: '', euStatus: 'ALL' },
    ...
    methods: {
        ...
        async fetchData() {
            ...
            try {
                ...
                this.stats          = {
                    ...
                    avgLoadTime:    data.stats?.avgLoadTime    ?? 0,
                    euShieldedCount: data.stats?.euShieldedCount ?? 0
                };
                this.updateTimeline();
            } catch (e) {
        ...
        resetFilters() {
            this.filters = { search: '', device: 'ALL', platform: 'ALL', browser: 'ALL', flag: 'ALL', dateFrom: '', dateTo: '', euStatus: 'ALL' };
            this.pagination.page = 1;
        ...
        exportCSV() {
            ...
            const hdr = ['Time','IP','City','Country','ISP','Device','OS','Browser','Page','Referrer','Flags','Viewport','DPR','Color Depth','Session Views','Load Time (ms)','EU Shielded'];
            const rows = this.logs.map(l =>
                [
                    this.formatDate(l.timestamp), l.ip, l.city, l.country, l.isp,
                    l.device, l.platform, l.browser, l.page, l.referrer,
                    (l.flags||[]).join('|'), l.vp||'—', l.dpr||'—', l.col||'—',
                    l.sessViews||'—', l.loadTime||'—', l.euScrubbed ? 'YES' : 'NO'
                ]
