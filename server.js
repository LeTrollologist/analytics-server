...
app.get('/api/telemetry', async (req, res) => {
    ...
    try {
        ...
        // Advanced Aggregations using the active filters
        const totalHits = await Log.countDocuments(match);
        const uniqueIPs = await Log.distinct('ip', match).then(ips => ips.length);
        
        const devices = await Log.aggregate([{ $match: match }, { $group: { _id: "$device", count: { $sum: 1 } } }]);
        const platforms = await Log.aggregate([{ $match: match }, { $group: { _id: "$platform", count: { $sum: 1 } } }]);
        ...
        res.json({ logs, stats: { totalHits, uniqueIPs, devices, platforms, locations, timeline } });
    } catch (err) { res.status(500).json({ error: "SYSTEM CRASH" }); }
});

// ---------------------------------------------------------
// 3. ADMIN DASHBOARD ROUTE (Serves the UI)
// ---------------------------------------------------------
app.get('/admin', (req, res) => {
    const secret = process.env.ADMIN_PW || "admin123";
    if (req.query.pw !== secret) return res.status(401).send("ACCESS DENIED");

    // We send a massive raw HTML string containing Vue.js and Tailwind
    res.send(`
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div id="app" class="p-4 md:p-8 max-w-[1800px] mx-auto min-h-screen flex flex-col">
        ...
        <header class="flex flex-col md:flex-row justify-between items-end border-b border-cyan-500/50 pb-4 mb-6">
            <div>
                <div class="text-[10px] text-cyan-400 opacity-70 uppercase mb-1 font-bold tracking-widest">Orbital Monitoring Station // V2.0</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="cyber-border p-4">
                <h3 class="text-xs text-cyan-400 mb-4 uppercase font-bold tracking-widest">Temporal Activity (7 Days)</h3>
                <div class="h-[200px]"><canvas id="timelineChart"></canvas></div>
            </div>
            <div class="cyber-border p-4 flex flex-col">
                <h3 class="text-xs text-cyan-400 mb-4 uppercase font-bold tracking-widest">Hardware / OS Distribution</h3>
            ...
            <div class="cyber-border p-4">
                <h3 class="text-xs text-pink-500 glow-pink mb-4 uppercase font-bold tracking-widest">Top Geolocation Sectors</h3>
                <div class="space-y-3">
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    ...
                    <tbody class="text-xs font-mono">
                        <tr v-if="logs.length === 0">
                            <td colspan="6" class="p-8 text-center text-gray-500">NO SIGNALS FOUND IN CURRENT MATRIX</td>
                        </tr>
                        <tr v-for="log in logs" :key="log._id" @click="openDeepScan(log)" class="border-b border-white/5 hover:bg-cyan-500/10 transition cursor-pointer group">
                            ...
                            <td class="p-3">
                                <span class="text-pink-400">{{ log.platform }}</span> <span class="text-gray-600">|</span> <span class="text-gray-400">{{ log.device }}</span>
                            </td>
        ...
        <div v-if="selectedLog" class="modal-overlay" @click.self="selectedLog = null">
            <div class="cyber-border modal-content p-6 shadow-2xl">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm font-mono mb-6">
                    <div><span class="text-gray-500 block text-[10px]">TARGET_IP</span><span class="text-cyan-400">{{ selectedLog.ip }}</span></div>
                    ...
                    <div class="col-span-2"><span class="text-gray-500 block text-[10px]">RAW_USER_AGENT</span><span class="text-gray-400 text-xs break-all">{{ selectedLog.ua }}</span></div>
                    
                    <div><span class="text-gray-500 block text-[10px]">ISP / ROUTER</span><span class="text-gray-200">{{ selectedLog.isp }}</span></div>
                    <div><span class="text-gray-500 block text-[10px]">COORDINATES</span><span class="text-gray-200">{{ selectedLog.city }}, {{ selectedLog.region }}, {{ selectedLog.country }}</span></div>
                    
                    <div><span class="text-gray-500 block text-[10px]">HARDWARE_NODE</span><span class="text-pink-400">{{ selectedLog.device }}</span></div>
                    <div><span class="text-gray-500 block text-[10px]">OS_KERNEL</span><span class="text-pink-400">{{ selectedLog.platform }}</span></div>
                    
                    <div><span class="text-gray-500 block text-[10px]">VIEWPORT_RES</span><span class="text-cyan-400">{{ selectedLog.screen }}</span></div>
                    <div><span class="text-gray-500 block text-[10px]">ACTIVE_PAGE</span><span class="text-cyan-400">{{ selectedLog.page }}</span></div>
                </div>
    ...
    <script>
        ...
        createApp({
            data() {
                return {
                    logs:[],
                    stats: { totalHits: 0, uniqueIPs: 0, devices: [], platforms:[], locations: [], timeline:[] },
                    filters: { search: '', device: 'ALL', platform: 'ALL' },
                    isLive: true,
                    loading: false,
                    pollInterval: null,
                    selectedLog: null,
            ...
            methods: {
                ...
                resetFilters() {
                    this.filters = { search: '', device: 'ALL', platform: 'ALL' };
                    this.fetchData();
                },
                toggleLive() {
                ...
                initCharts() {
                    ...
                    this.charts.platform = new Chart(platCtx, {
                        type: 'doughnut',
                        data: { labels: [], datasets: [{ data: [], backgroundColor:['#00f3ff', '#ff00ff', '#8b5cf6', '#10b981'], borderWidth: 0 }] },
                        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, cutout: '70%' }
                    });
                },
                updateCharts() {
